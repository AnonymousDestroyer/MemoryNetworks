...
parser.add_argument('--n_epochs', type=int, default=10, help='default 100. the number of epochs')#befor diy value = 100 now =5
...
def train(model,train_data,test_data,optimizer,loss_fn,w2i,task_id,batch_size,n_epoch):
    for epoch in range(n_epoch):
        print('epoch by me {} start---'.format(epoch))
        model.train()
        correct = 0
        count =0
        ...
        if epoch % 2 ==0:
            ...
            print('Training Acc: {:.2f}% - '.format(correct.__float__()/count*100),correct,'/',count)
